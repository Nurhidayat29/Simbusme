	...
	<head>
		...
		<title>
			<?=$this->setting->login_title
				...
				. (($desa['nama_kabupaten']) ? ' ' . $desa['nama_kabupaten']: '')
				...
				<div class="container">
					<div class="row">
						<div class="col-sm-6 col-sm-offset-3 form-box">
							<div class="form-top">
								<a href="<?=site_url(); ?>first/"><img src="<?=LogoDesa($desa['logo']);?>" alt="<?=$desa['nama_kabupaten']?>" class="img-responsive" /></a>
								<div class="login-footer-top"><h1><?=ucwords($this->setting->sebutan_kabupaten)?> <?=$desa['nama_kabupaten']?></h1>
									<h3>
										<br /><?=$desa['alamat_kantor']?><br />Kodepos <?=$desa['kode_pos']?>
										<br /><?=ucwords($this->setting->sebutan_kecamatan)?> <?=$desa['nama_kecamatan']?><br /><?=ucwords($this->setting->sebutan_desa)?> <?=$desa['nama_desa']?><br /><?=ucwords($this->setting->sebutan_provinsi)?> <?=$desa['nama_propinsi']?>
									</h3>
								</div>
								<hr />
							</div>
								...
								<form class="login-form" action="<?=site_url('siteman/auth')?>" method="post" >
									...
									<?php else: ?>
										...
										<?php elseif ($_SESSION['siteman']==-2): ?>
											<div class="error">
												Redaksi belum boleh login, SimBusMe belum memiliki sambungan internet!
											</div>
										<?php endif; ?>
									<?php endif; ?>
								</form>
								<hr/>
								<div class="login-footer-bottom">powered by: <a href="https://github.com/OpenSID/OpenSID" target="_blank">SimBusMe</a> <?= substr(AmbilVersi(), 0, 11)?></div>
							</div>
						</div>
					</div>
				</div>
