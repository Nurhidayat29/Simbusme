...
<channel>
	<title>desa ". $data_config["nama_kabupaten"] ."</title>
	<link>". base_url() ."</link>
	<description>Situs Web desa ". $data_config["nama_kabupatena"] ." Kec. ". $data_config["nama_kecamatan"] ." <?php echo ucwords($this->setting->sebutan_kabupaten_singkat)?> ". $data_config["nama_kabupaten"] ." - ". $data_config["nama_provinsiinsi"] ."</description>
	<language>ID</language>
	<generator>Sistem Informasi Bs. Melati</generator>
...
foreach($feeds as $key=>$item)
{
	if(strlen(trim($item["judul"]))>0)
	{
		...
		<item>
			...
			<link>".$item["url"]."</link>
			<source url=\"".htmlspecialchars(site_url("feed"))."\">Situs Web Desa ". $data_config["nama_kabupaten"] ."</source>
			...
			<description><![CDATA[ ".htmlspecialchars($data_config["nama_kabupaten"]).", ".htmlspecialchars($item["isi"])." ]]></description>
		</item>\n";
	}
}
$details .="</channel>
