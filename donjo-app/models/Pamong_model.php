<?php class Pamong_model extends CI_Model {
	...
	public function update($id=0)
	{
		...
		$data = array();
		$data = $this->siapkan_data($data);
		$this->db->where("id", $id)->update('tweb_desa_pamong', $data);
	}

	public function delete($id='')
	{
		$foto = $this->db->select('foto')->where('id',$id)->get('tweb_desa_pamong')->row()->foto;
		if (!empty($foto))
		{
			unlink(LOKASI_USER_PICT.$foto);
			unlink(LOKASI_USER_PICT.'kecil_'.$foto);
		}
		$sql = "DELETE FROM tweb_desa_pamong WHERE id = ?";
	...
	public function ttd($id='', $val=0)
	{
		...
		$this->db->where('id', $id)->update('tweb_desa_pamong', array('pamong_ttd'=>$val));
	}
  ...
  private function urut_max()
  {
	...
	public function urut($id, $arah)
	{
		...
		// Tukar urutan
		$this->db->where('id', $pamong2['id'])->
			update('tweb_desa_pamong', array('urut' => $pamong1['urut']));
		$this->db->where('id', $pamong1['id'])->
