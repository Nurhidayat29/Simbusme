...
class Sid_Core extends Admin_Controller {
  ...
  private function urut_max()
  {
	...
	{
		$sql = "SELECT urut, COUNT(*) c FROM tweb_wil_clusterdesa GROUP BY urut HAVING c > 1";
		...
		if ($urut_duplikat)
		{
			...
			$q = $this->db->get('tweb_wil_clusterdesa');
			$wilayah = $q->result_array();
			for ($i=0; $i<count($wilayah); $i++)
			{
				$this->db->where('id', $wilayah[$i]['id']);
				$data['urut'] = $i + 1;
				$this->db->update('tweb_wil_clusterdesa', $data);
			}
		}
	}
	...
	public function urut($id, $arah)
	{
		...
		if ($arah == 1)
		{
			if ($i >= count($wilayah) - 1) return;
			$wilayah2 = $wilayah[$i + 1];
		}
		if ($arah == 2)
		{
			if ($i <= 0) return;
		...
		// Tukar urutan
		$this->db->where('id', $wilayah2['id'])->
			update('tweb_wil_clusterdesa', array('urut' => $wilayah1['urut']));
		$this->db->where('id', $wilayah1['id'])->
			update('tweb_wil_clusterdesa', array('urut' => $wilayah2['urut']));
	}
